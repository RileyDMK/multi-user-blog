...
class Handler(webapp2.RequestHandler):
    """Base handler that defines rendering and cookie functions for child handlers."""
    def write(self, *a, **kw):
...
class Blog(db.Model):
    """Handles creation of blog entities."""
...
class Comment(db.Model):
    """Handles creation of comment entities."""
    content = db.TextProperty(required = True)
    created = db.DateTimeProperty(auto_now_add = True)
...
class Signup(Handler):
    """Defines the form validation for the register handler."""
    def get(self):
...
class BlogPage(Handler):
    ...
    #Checks for type of submission and follows up with the appropriate response.
...
class CommentHandler(Handler):
    ...
    def post(self, blog_id):
        ...
        if content:
            c = Comment(content = content, user_id = int(user_id), blog_id=int(blog_id), blog=blog)
            c.put()
            time.sleep(0.1)
            link_id = blog.key().id()
...
class EditComment(Handler):
    ...
    def get(self, comment_id, content="", error=""):
        ...
        content = c.content
        if author:
            self.render('editcomment.html', content=content, error=error, author=int(author))
        else:
...
class EditPost(Handler):
    ...
    def get(self, blog_id, subject="", content="", error=""):
        ...
        content = b.content
        if author:
            self.render('edit.html', blog_id=blog_id, subject=subject, content=content, error=error, author = int(author))
        else:
